...
//sdtmsapp01v  10.42.18.207
//sdtmsapp02v  10.42.18.208
//sdtmsapp03v  10.42.18.209 
//sdtmsapp04v  10.42.18.210

emailDistribution="ppiermarini@incomm.com"
//General pipeline 

artifactDeploymentLoc="/opt/pivotal/pivotal-tc-server-4.0.11.RELEASE/instances/tms-instance/webapps/"
libDeploymentLoc="/opt/pivotal/pivotal-tc-server-4.0.11.RELEASE/instances/tms-instance/lib/"
instanceLogs="/opt/pivotal/pivotal-tc-server-4.0.11.RELEASE/instances/tms-instance/logs"
		...
		stage('Build'){
			...
			if (userInput == 'Build') {
				...
				withSonarQubeEnv('sonarqube.incomm.com'){
					...
					sh "cp tms-service-webapp/target/${artifactName} . "
				}
...
stage('SonarQube Quality Gate Check') {
	...
	stage('Deployment'){
		...
		if ((userInput == 'Build') || (userInput == 'Promote')) {
			...
			sh"cp ${artifactId}-${artifactVersion}.${artExtension} ${artifactName}"
			sh"ls -ltr"
...
def deploy(target_hostname, artifactDeploymentLoc, Artifact) {
	echo "Artifact being deployed ${Artifact}"
	...
	sh """
		...
		ssh -q -o StrictHostKeyChecking=no root@${target_hostname} 'rm -rf $artifactDeploymentLoc/tms-service-v2.x/'
