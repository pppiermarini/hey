...
gitRepository = "https://github.com/InComm-Software-Development/cfes-tms-fraudui.git"
//gitBranch = "${Branch}"
gitCreds = "scm-incomm"
...
targets = [
	'dev': ['10.42.18.212','10.42.18.213','10.42.18.214','10.42.18.215'],
	'qa': ['sqfrdapp01v','sqfrdapp02v','sqltmsfrdapp01v','sqltmsfrdapp02v'],
	'prod_1': ['sptmsfrdapp01v'],
	'prod_2' : ['sptmsfrdapp02v'],
	'prod_3' : ['sptmsfrdapp03v']
]

emailDistribution="ppiermarini@incomm.com"
//General pipeline 

artifactDeploymentLoc = "/opt/pivotal/pivotal-tc-server-4.0.11.RELEASE/instances/fraud-ui-instance/webapps"
libDeploymentLoc="/opt/pivotal/pivotal-tc-server-4.0.11.RELEASE/instances/fraud-ui-instance/lib"
instanceLogs="/opt/pivotal/pivotal-tc-server-4.0.11.RELEASE/instances/fraud-ui-instance/logs"
def artifactloc = "${env.WORKSPACE}"

serviceName = "fraud-ui-instance"
...
artifactId ='tms-webapp-fraud-harness'
env_propertyName ='ART_VERSION'
artExtension ='war'
artifactName ='tms-fraud-ui.war'

artifactRealName = 'tms-fraud-ui'
...
node('linux1'){
	...
	try {
		...
		stage('checkout'){
			...
			echo "Checking out p${gitBranch}p"
			cleanWs()
		...
		stage('Build'){
			...
			if (userInput == 'Build') {
				...
				sh "cp tms-webapp-fraud-harness/target/${artifactName} . "
...
*/
		...
		stage('Notification'){
			...
			sendEmail(emailDistribution, userInput, target_env, userApprove)
