...
gitRepository = "https://github.com/InComm-Software-Development/cfes-tmsfs-fraudmodule.git"
gitBranch = "${Branch}"
gitCreds = "scm-incomm"
...
instanceLogs="/var/opt/pivotal/pivotal-tc-server-standard/tmsfs-fraud-ws-instance/logs"
	...
	try {
		...
		stage('Build'){
			...
			if (userInput == 'Build') {
				...
				sh "cp tmsfs-fraud-service/target/${artifactName} . "
...
def deploy(target_hostname, artifactDeploymentLoc, Artifact) {
	...
	sh """
		...
		scp -r -q -o StrictHostKeyChecking=no configurations/${myenv}/${star} root@${target_hostname}:${libDeploymentLoc}
		scp -r -q -o StrictHostKeyChecking=no configurations/${myenv}/${prop} root@${target_hostname}:${libDeploymentLoc}
		scp -r -q -o StrictHostKeyChecking=no configurations/${myenv}/${xml} root@${target_hostname}:${libDeploymentLoc}
