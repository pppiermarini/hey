...
def deploy(target_hostname, artifactDeploymentLoc, Artifact) {
	...
	sh """
		...
		ssh -i ~/.ssh/pipeline -q -o StrictHostKeyChecking=no root@${target_hostname} 'rm -f $artifactDeploymentLoc/${artifactName}'
		ssh -i ~/.ssh/pipeline -q -o StrictHostKeyChecking=no root@${target_hostname} 'rm -rfv $artifactDeploymentLoc/${artifactId}'
		scp -q -i ~/.ssh/pipeline ${artifactName} root@${target_hostname}:$artifactDeploymentLoc/
		ssh -q -i /app/jenkins/.ssh/pipeline root@${target_hostname} '/bin/chown -R ${USER}:${GROUP} $artifactDeploymentLoc/${artifactName}'
	"""
	}
}
