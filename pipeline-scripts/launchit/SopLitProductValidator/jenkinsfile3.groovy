...
gitRepository = "https://github.com/InComm-Software-Development/sop-lit-productvalidator.git"
gitBranch = "${Branch}"
gitCreds = "scm-incomm"
...
emailDistribution="rkantareddy@incomm.com"
//General pipeline 

artifactDeploymentLoc = "/var/opt/lit-pv-service"
...
artifactRealName = 'lit-pv-service.jar'
		...
		stage('Build'){
			...
			if (userInput == 'Build') {
				...
				//withSonarQubeEnv('sonarqube.incomm.com'){
					...
					sh "cp target/${artifactId}*.${artExtension} ${artifactRealName}"
				//}
...
//		}
		...
		artifactWget(repoId, groupId, artifactId, artExtension, artifactVersion)
			...
			sh "mv ${artifactId}-${artifactVersion}.${artExtension} ${artifactRealName}"
		...
		stage('Deployment'){
			...
			if ((userInput == 'Build') || (userInput == 'Promote')) {
				...
				deployComponents(target_env, targets[target_env], "${artifactRealName}")
...
def deploy(target_hostname, artifactDeploymentLoc, Artifact) {
	...
	sshagent([credsName]) {
		sh """
			...
			scp -q -o StrictHostKeyChecking=no ${artifactRealName} root@${target_hostname}:$artifactDeploymentLoc/
			ssh -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/chown -R ${user}:${group} $artifactDeploymentLoc/${artifactRealName}'
			ssh -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/chmod -R ${chmodValue} $artifactDeploymentLoc/${artifactRealName}'
