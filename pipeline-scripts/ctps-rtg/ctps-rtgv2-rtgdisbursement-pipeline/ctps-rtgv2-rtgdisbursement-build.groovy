...
gitRepository="https://github.com/InComm-Software-Development/ctps-rtgv2-rtgdisbursement.git"
gitBranch="${Branch}"
gitCreds="scm-incomm"
...
emailDistribution="shaque@incomm.com rkale@incomm.com vhari@incomm.com dstoval@incomm.com skeshari@InComm.com mmurarisetty@incomm.com stadoori@incomm.com"
...
imageName = "rtgv2-rtgdisbursement"
	...
	try { 
		...
		stage('Build-Release'){
			...
			if (userInput == 'Build'){
				...
				//sh "${maven} -B -f pom.xml -U -X clean compile sonar:sonar jib:build"
				sh"${maven} clean deploy -f ${env.WORKSPACE}/pom.xml -e -U -X -DskipTests sonar:sonar -Dsonar.branch.name=${BRANCH}"
				}
