...
gitRepository = "https://github.com/InComm-Software-Development/ccl-cache.git"
gitBranch = "${Branch}"
gitCreds = "scm-incomm"
...
node('linux'){
	try {
		...
		stage('Build'){
			...
			if (userInput == 'Build') {
				...
				withSonarQubeEnv('sonarqube.incomm.com'){
					sh "${maven} clean deploy -f pom.xml -U"
		...
		stage ('Artifact scan'){
			...
			def artifactoryServer = Artifactory.newServer url: 'https://maven.incomm.com/artifactory' , credentialsId: 'cruise'
