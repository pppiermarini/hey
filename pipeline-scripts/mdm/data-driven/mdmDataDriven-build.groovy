...
pomPath = "pom.xml"
reports = []
emailDistribution = "jrivett@incomm.com, mmdmdevelopmentteam@incomm.com"
...
node('linux2'){
	try { 
		...
		// DEPRECATED!!! DO NOT DO SNYK SCANS LIKE THIS!!!
        stage('Snyk Scan'){
			scanCodeDir = "${env.WORKSPACE}/reports/"

			// Authenticate snyk
			echo "Snyk auth for SOP"
			withCredentials([string(credentialsId: 'snyk-sop', variable: 'AUTH')]) {
				sh "${snyk} auth ${AUTH}"
			}
		...
		stage('Build'){
			...
			if (pipelineAction == 'build'){
				...
				sh "${maven} clean deploy -f ${pomPath} -DskipTests"
			}
