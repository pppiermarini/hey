...
node('linux2'){
    ...
    stage('Schedule the Deployment'){
		if (pipelineAction == 'Create-Change'){
			...
			sshagent([credsName]) {
				sh """
				echo "Tic"
				ssh -q -o StrictHostKeyChecking=no root@${script_hostname} 'sh -c "( ( /home/ppiermarini/launchJob.sh 120 ${NEW_VERSION} Deploy ${target_env} &>/dev/null ) & )"'
				echo "Toc"
				"""
			}
    ...
    stage('Cancel Scheduled Deployment'){
		...
		if (pipelineAction == 'Cancel-Change'){
			sshagent([credsName]) {
				sh """
