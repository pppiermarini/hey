...
emailDistribution="dstovall@incomm.com ppiermarini@incomm.com JRivett@incomm.com"
//General pipeline
		...
		stage('Build'){
			...
			if (userInput == 'Build'){
				...
				withSonarQubeEnv('sonarqube.incomm.com'){
					sh "${maven} clean deploy -f pom.xml -e -U -Dmaven.javadoc.skip=true --batch-mode sonar:sonar"
				...
				sh "${maven} org.apache.maven.plugins:maven-release-plugin:2.5.3:prepare -Darguments='-Dmaven.javadoc.skip=true'"
				sleep(3)
				sh "${maven} org.apache.maven.plugins:maven-release-plugin:2.5.3:perform -Darguments='-Dmaven.javadoc.skip=true'"
				sleep(4)
