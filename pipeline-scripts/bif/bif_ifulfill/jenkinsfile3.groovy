...
gitRepository = "https://github.com/InComm-Software-Development/bif-ifulfill"
gitBranch = "${Branch}" 
gitCreds = "scm-incomm"
...
artifactDeploymentLoc = "C\$\\bif-ifulfill-ms\\bif-ifulfill-ms"
		...
		stage('Build'){
			...
			if (userInput == 'Build') {
				...
				//withSonarQubeEnv('sonar'){
					bat "${maven} clean deploy -X -e -U -B -DskipTests sonar:sonar"
//                    		//bat "${maven} clean deploy -X -e -U -B -DskipTests sonar:sonar"
...
//move \\\\${target_hostname}\\C\$\\bif-ifulfill-ms\\bif-ifulfill-ms\\${artifactName} \\\\${target_hostname}\\D\$\\incomm\\backup
//del \\\\${target_hostname}\\c\$\\bif-ifulfill-ms\\${artifactName}
