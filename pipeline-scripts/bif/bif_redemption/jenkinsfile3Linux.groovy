...
gitRepository = "https://github.com/InComm-Software-Development/bif-redemption"
gitBranch = "${Branch}"
gitCreds = "scm-incomm"
...
artifactDeploymentLoc = "/var/BIFCoreServices/bif-redemption-writer-ms/"
instanceLogs="/opt/BIFCoreServices/bif-redemption-writer-ms/logs"
...
serviceName = "bif-redemption-writer"
...
artifactId = 'bif-redemption-writer'
env_propertyName = 'ART_VERSION'
artExtension = 'jar'
artifactName = 'bif-redemption-writer.jar'
	...
	try {
		...
		stage('Build'){
			...
			if (userInput == 'Build') {
				...
				sh "${maven} clean deploy -DskipTests=true -B"
...
def deploy(target_hostname, artifactDeploymentLoc, Artifact) {
	...
	sshagent([credsName]) {
		sh """
			...
			ssh -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/chown bifcore:bifcore ${artifactDeploymentLoc}/${artifactName}'
