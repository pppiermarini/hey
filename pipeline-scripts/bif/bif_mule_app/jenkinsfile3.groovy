...
muleLoc = 'C$\\mule\\mule-standalone-3.3.1\\bin'
		...
		stage('Build'){
			...
			if (userInput == 'Build') {
				...
				//withSonarQubeEnv('sonar'){
					bat "${maven} clean deploy -X -f pom.xml -U -B -DskipTests sonar:sonar"
//					bat "${maven} clean deploy -X -f pom.xml -U -B -DskipTests sonar:sonar"

					pom = readMavenPom file: 'pom.xml'
					relVersion = pom.getVersion();

					bat "copy bif-mule-app\\target\\${artifactId}-${relVersion}.zip ."
...
//get-service -Name $env:serviceName -ComputerName sdbifapp283v.uss.net | Where-Object {$_.Status -eq "Stopped"}) {"Success"} else {"fail"} 
...
//del \\\\${target_hostname}\\${artifactDeploymentLoc}\\bif-mule*
//powershell Remove-Item \\\\${target_hostname}\\c\$\\mule\\mule-standalone-3.3.1\\apps\\bif-mule-app* -Force -Recurse
//powershell Get-Service ${serviceName} -ComputerName ${target_hostname} ^| Start-Service
//powershell Invoke-Command -ComputerName ${target_hostname} {taskkill /F /FI 'Services eq ${serviceName}'}
//powershell Invoke-Command -ComputerName ${target_hostname} -ErrorAction Stop -ScriptBlock {Invoke-Expression -Command:"cmd.exe /c 'C:\\mule\\mule-standalone-3.3.1\\bin\\mule.bat stop'"}
//powershell Invoke-Command -ComputerName ${target_hostname}\\${muleLoc}\\${serviceName} Stop
