...
gitRepository = "https://github.com/InComm-Software-Development/BIFDataAccessService.git"
gitBranch = "${Branch}"
gitCreds = "scm-incomm"
...
emailDistribution="ewaktola@incomm.com ybirru@incomm.com"
//General pipeline .

artifactDeploymentLoc = "/var/BIFCoreServices/bif-data-access-ms/"
instanceLogs="/opt/BIFCoreServices/bif-data-access-ms/logs"
...
serviceName = "BIFDataAccessService"
...
artifactId = 'BIFDataAccessService'
env_propertyName = 'ART_VERSION'
artExtension = 'jar'
artifactName = 'BIFDataAccessService.jar'
	...
	try {
		...
		stage('Build'){
			...
			if (userInput == 'Build') {
				...
				sh "${maven} clean deploy -DskipTests=true -B sonar:sonar"
				pom = readMavenPom file: 'pom.xml'
				relVersion = pom.getVersion();
				sh "cp ${env.WORKSPACE}/target/${artifactId}-${relVersion}.jar ${env.WORKSPACE}/${artifactId}.jar"
...
def deploy(target_hostname, artifactDeploymentLoc, Artifact) {
	...
	sshagent([credsName]) {
		sh """
			...
			ssh -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/chown bifdataaccess:bifdataaccess ${artifactDeploymentLoc}/${artifactName}'
