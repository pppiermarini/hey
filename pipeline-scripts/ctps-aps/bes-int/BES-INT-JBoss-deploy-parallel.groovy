...
xmlSources = []
xmlDestinations = []
serviceName = 'JBossEAP7.2-BESINT'
    ...
    try {  
        ...
        stage('get-artifacts'){
            ...
            if ("${DEPLOY_ARTS}" == 'true') {
                ...
                def stepsInParallel = yml.deployments.collectEntries {["$it": {
                    currentSubStack = readYaml(text: "${it}");
                    echo "${currentSubStack.artifacts}";
                }]}
                parallel stepsInParallel
                inputRequested([""])

                // def stepsInParallel =  properties.collectEntries {
                //     [ "$it" : {echo it} ]
                // }
                // parallel stepsInParallel
...
def downloadSubStack(properties) {
    echo "location: ${properties.location}"
    echo "artifacts: ${properties.artifacts}"
