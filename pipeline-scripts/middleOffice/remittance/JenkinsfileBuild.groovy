...
gitRepository="https://github.com/InComm-Software-Development/ipbr-remittance.git"
gitBranch="${Branch}"
gitCreds="scm-incomm"
...
emailDistribution="tmalik@incomm.com ppiermarini@incomm.com"
		...
		stage('Build'){
			...
			if (userInput == 'Build'){
				...
				withSonarQubeEnv('sonarqube.incomm.com'){
					...
					sh "${maven} clean deploy -f ${folder}/pom.xml -DskipTests sonar:sonar"
				}
				...
				pom = readMavenPom file: "${folder}/pom.xml"
				def mavenReleaseVersion = pom.getVersion();
				echo " maven release= " + "${mavenReleaseVersion}"
				
				sh "${maven} -f ${folder}/pom.xml org.apache.maven.plugins:maven-release-plugin:2.5.3:prepare"
				sleep(3)
				sh "${maven} -f ${folder}/pom.xml org.apache.maven.plugins:maven-release-plugin:2.5.3:perform"
				sleep(4)
