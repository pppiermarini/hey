...
gitRepository="https://github.com/InComm-Software-Development/ipbr-cache-daemon.git"
gitBranch="${BRANCH}"
gitCreds="scm-incomm"
folder="mo-cache"
...
//General pipeline
emailDistribution="ppiermarini@incomm.com tmalik@incomm.com"
...
release_folder = "mo-cache/pom.xml"
		...
		def	pomVersion = pom.getVersion();
			...
			if (userInput == 'Build'){
				...
				withSonarQubeEnv('sonarqube.incomm.com'){
					...
					sh "${maven} clean deploy -DskipTests sonar:sonar -DenvName=${targetEnv} -Dsonar.branch.name=${BRANCH}"
					//sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectKey=${sonarProjectName} -Dsonar.sources=app/"
					sleep 3
					//sh "${maven} -X deploy:deploy-file"
				}

				// deploying the archive packages to Artifactory
				if(pomVersion.contains('SNAPSHOT')){
					echo "contains SNAPSHOT"
				sh "mv target/cachedaemon-dist.zip cachedaemon-dist-${pomVersion}.zip"
				sh "mv target/cachedaemon-dist.tar.gz cachedaemon-dist-${pomVersion}.tar.gz"
				sh "/usr/bin/jfrog rt u '*.zip' incomm-snapshot/com/incomm/middleoffice/cache/cachedaemon/${pomVersion}/dist/"
				sh "/usr/bin/jfrog rt u '*.gz' incomm-snapshot/com/incomm/middleoffice/cache/cachedaemon/${pomVersion}/dist/"
				}else{
					echo "contains RELEASE"
				sh "mv target/cachedaemon-dist.zip cachedaemon-dist-${pomVersion}.zip"
				sh "mv target/cachedaemon-dist.tar.gz cachedaemon-dist-${pomVersion}.tar.gz"
				sh "/usr/bin/jfrog rt u '*.zip' incomm-release/com/incomm/middleoffice/cache/cachedaemon/${pomVersion}/dist/"
				sh "/usr/bin/jfrog rt u '*.gz' incomm-release/com/incomm/middleoffice/cache/cachedaemon/${pomVersion}/dist/"					
				}
				...
				pom = readMavenPom file: 'mo-cache/pom.xml'
