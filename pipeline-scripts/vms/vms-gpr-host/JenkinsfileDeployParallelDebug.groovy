...
emailDistribution="FSCorePlatforms-Dev@incomm.com"
//General pipeline 
...
def deploy(target_hostname, artifactDeploymentLoc, Artifact) {
    ...
    sh """
		...
		sleep 60
		ssh -i ~/.ssh/pipeline -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/find ${artifactDeploymentLoc} -type f -name ${artifactName}*.failed | wc -l' > ${target_hostname}_commandresult
		sleep 10
	"""


	def r = readFile('${target_hostname}_commandresult').trim()
			echo "arr= p${r}p"
			if(r == "1"){
			...
			timeout(10) {
				waitUntil {
						sh """ssh -i ~/.ssh/pipeline -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/find ${artifactDeploymentLoc} -type f -name ${artifactName}*.deployed | wc -l' > ${target_hostname}_commandresult"""
						def a = readFile('${target_hostname}_commandresult').trim()
						echo "arr= p${a}p"
						if (a == "1"){
						return true;
						}else{
