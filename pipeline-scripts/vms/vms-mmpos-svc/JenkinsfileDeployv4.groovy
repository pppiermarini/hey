...
gitRepository ="https://github.com/InComm-Software-Development/vms-mmpos-svc-props.git"
gitBranch="main"
gitCreds = "scm-incomm"
...
artifactDeploymentLoc ="/srv/jboss-eap-7.3/standalone_mmpos/deployments"
propDeploymentLoc="/home/InComm/vms-mmpos-svc/properties"
srcproperties="/app/jenkins/workspace/vms_mmpos_svc/vms-mmpos-svc-deploy"
serviceName="jboss-as-standalone_mmpos"
pipeline_id="${env.BUILD_TAG}"
tmpLocation="/srv/jboss-eap-7.3/standalone_mmpos/tmp"
dataLocation="/srv/jboss-eap-7.3/standalone_mmpos/data"
...
artifactId = 'vms-mmpos-svc'
env_propertyName = 'ART_VERSION'
artExtension = 'war'
artifactName = 'vms-mmpos-svc.war'
ArtifactVersion = ""
list=""
...
def deploy(target_hostname, artifactDeploymentLoc, artifactName) {
	...
	sh """
		...
		ssh -o StrictHostKeyChecking=no root@${target_hostname} '/bin/rm -rf ${artifactDeploymentLoc}/vms-mmpos-svc*'
		...
		ssh -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/su - jboss '/home/InComm/vms-mmpos-svc/start-mmpos-svc.sh' > /dev/null'
	"""
	}
...
def stop(target_hostname, serviceName, dataLocation, tmpLocation) {
	...
	sh """
		...
		ssh -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/sh /home/InComm/vms-mmpos-svc/stop-mmpos-svc.sh > /dev/null || true'
		ssh -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/chown -R jboss:jboss /home/InComm/vms-mmpos-svc/logs*'
		sleep 10
		"""
		}
}
...
def start(target_hostname, serviceName, dataLocation, tmpLocation) {
	...
	sh """
		ssh -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/chmod 755 /srv/jboss-eap-7.3/bin/jboss-cli.sh > /dev/null || true'
		ssh -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/su jboss -c '/home/InComm/vms-mmpos-svc/start-mmpos-svc.sh' > /dev/null'
		ssh -q -o StrictHostKeyChecking=no root@${target_hostname} '/bin/chown -R jboss:jboss /home/InComm/vms-mmpos-svc/logs*'
		sleep 10
		"""
		}
