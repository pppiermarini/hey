...
JBossStartup="/srv/jboss-eap-6.3/bin/jboss_startup_aps.sh"
JBossShutdown="/srv/jboss-eap-6.3/bin/jboss_shutdown_aps.sh"
...
groupId = 'com.incomm.uss.lib'
artifactId = 'uss-greencard-service'
...
//wget --user "${ArtUser}" --password "${ArtPass}" -O ${artifactName} --no-check-cert --no-verbose http://maven.incomm.com/artifactory/repo/\$groupIdDir/ivr/${artifactVersion}/ivr-${artifactVersion}.war"
		...
		stage('Deployment'){
			...
			//md5SumCheck(targets[targetEnv],DeployLocationmain,"${artifactName},"${artifactName}")
...
def deployComponents(envName, targets, Artifact){
	...
	def stepsInParallel =  targets.collectEntries {
		[ "$it" : { deploy(it, DeployLocationmain, Artifact) } ]
	}
