...
def deploy(target_hostname, artifactDeploymentLoc, Artifact) {
	...
	sshagent([credsName]) {
		sh """
			echo "stopping jboss..."
			ssh -q -o StrictHostKeyChecking=no root@${target_hostname} 'sudo -u jboss /srv/jboss-eap-6.3/bin/jboss_shutdown_aps.sh'
			...
			ssh -q -o StrictHostKeyChecking=no root@${target_hostname} 'if [ ! -f "${Deploylocationlog}/server.log.${tstamp}" ]; then sudo -u jboss /bin/mv ${Deploylocationlog}/server.log server.log.${tstamp} stop; fi'
