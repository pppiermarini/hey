...
gitRepository="https://github.com/InComm-Software-Development/ctps-spil-services-vix.git"
gitBranch="${Branch}"
gitCreds="scm-incomm"
...
node('docker'){
	try { 
        ...
        stage ('Artifactory configuration') {
            ...
            artifactoryServer = Artifactory.newServer url: 'https://maven.incomm.com/artifactory' , credentialsId: 'dockerdeploy'

            rtMaven = Artifactory.newMavenBuild()
	    rtMaven.tool = "${maven}"
            rtMaven.deployer releaseRepo: docker, snapshotRepo: docker, server: artifactoryServer
            rtMaven.resolver releaseRepo: docker, snapshotRepo: docker, server: artifactoryServer
		...
		stage('Build-Release'){
			...
			if (userInput == 'Build'){
				...
				rtMaven.run pom: 'pom.xml', gaoals: 'clean compile' ,  jib:build, buildInfo: buildInfo
